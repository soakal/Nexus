...
async def fetch() -> GitHubData:
    ...
    async with httpx.AsyncClient(timeout=5) as client:
        ...
        # Raise on failure rather than reporting an empty list: a dead token (401)
        # or rate limit is otherwise indistinguishable from a genuinely quiet week,
        # and the briefing renders the empty list as "0 open PRs" fact either way —
        # the same zero-defaulting lesson every other integration already adopted
        # (unraid.py, proxmox.py, channels_dvr.py each raise on a failed core read).
        if resp.status_code != 200:
            raise RuntimeError(f"GitHub open-PRs search returned {resp.status_code}")
        open_prs = []
        stale_prs = []
        ...
        if resp2.status_code != 200:
            raise RuntimeError(f"GitHub assigned-issues fetch returned {resp2.status_code}")
        assigned_issues = []
        ...
        if resp3.status_code != 200:
            raise RuntimeError(f"GitHub events fetch returned {resp3.status_code}")
        recent_commits = []
