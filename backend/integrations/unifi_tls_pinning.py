"""TOFU (trust-on-first-use) TLS certificate pinning for the UniFi controller.

UniFi's local API normally only has a self-signed LAN certificate, so
validating it against public CAs was never an option -- which is why
`backend/integrations/unifi.py` used to construct its httpx client with
`verify=False`. That disables certificate validation entirely: any
network-adjacent attacker able to intercept the TCP session can present
*any* certificate and the client will happily complete the handshake and
send the login POST (`settings.unifi_username`/`settings.unifi_password`)
straight to the attacker (finding F15).

This module replaces the blanket `verify=False` with the same trust model
SSH's `known_hosts` uses:

* First successful connection to a given "host:port" -- the certificate's
  SHA-256 fingerprint is captured and persisted to `.unifi_known_hosts.json`
...
* Every later connection -- the presented certificate's fingerprint must
  ...
  below), with a `UnifiCertificateMismatchError` explaining the mismatch and
...
point control passes through before any HTTP data (including the login
POST) is sent, so a mismatch is refused pre-credential-exposure, not after.
"""
...
KNOWN_HOSTS_PATH = Path(".unifi_known_hosts.json")
...
class UnifiCertificateMismatchError(Exception):
    """A UniFi controller presented a TLS certificate that does not match the
    one pinned in .unifi_known_hosts.json on a prior connection to the same
    host:port.

    This is a fail-closed refusal: the connection is aborted before any HTTP
    data (including the login POST carrying the admin password) is written
    to the wire. If the mismatch is expected -- the controller's certificate
    was legitimately regenerated, or the controller/its network path was
    replaced -- deliberately re-trust the new certificate the same way you'd
    edit SSH's known_hosts: delete that host's entry (or the whole file)
    from .unifi_known_hosts.json, then reconnect to pin the new certificate.
    """
...
def _load_known_hosts(path: Path) -> dict:
    try:
        ...
        logger.warning("unifi known_hosts file %s unreadable (%s); treating as empty", path, exc)
        return {}
...
def _pin_or_verify(host_key: str, fingerprint: str, path: Path) -> None:
    ...
    `UnifiCertificateMismatchError` (fail closed) if the fingerprint no
    ...
    with _LOCK:
        ...
        if pinned is None:
            ...
            logger.info("unifi: pinned new TLS certificate for %s (trust-on-first-use)", host_key)
            return
        if pinned != fingerprint:
            raise UnifiCertificateMismatchError(
                f"TLS certificate mismatch for UniFi controller {host_key}: "
                ...
                "any request data (including login credentials). If this "
                "certificate change is expected (controller replaced / cert "
...
class _PinningNetworkStream(httpcore.AsyncNetworkStream):
    ...
    async def start_tls(
        ...
        except UnifiCertificateMismatchError:
            await tls_stream.aclose()
            raise
...
class PinnedTLSTransport(httpx.AsyncBaseTransport):
    """An httpx async transport that accepts a self-signed UniFi controller
...
def build_transport() -> PinnedTLSTransport:
    """Factory shared by unifi.py's three httpx.AsyncClient call sites
    (fetch, health_check, _stamgr_cmd) so all of them pin/verify identically."""
