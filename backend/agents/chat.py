...
# Structured-outputs schema for the intent classify call below. Added
# 2026-08-28 alongside router.py's response_schema plumbing -- constrains
# Haiku's output to guaranteed-valid JSON on models that support it
# (router._STRUCTURED_OUTPUT_MODELS), so the existing find("{")/json.loads
# parse below never has real fence-stripping/malformed-JSON work to do on
# those models. Left attached unconditionally: router.haiku() silently drops
# it for an unsupported model, so the existing parse stays the real fallback
# path there, unchanged.
_INTENT_SCHEMA = {
    "type": "object",
    "properties": {
        "intent": {
...
def _format_open_flags_for_memory(flags: list) -> str:
    ...
    docs/outcome-tracker-spec.md §4.3). Returns "" on an empty/all-invalid
    ...
    for f in flags:
        ...
        lines.append(f"#{f.get('id')} [{severity}] {source}:{check} — {summary}")
    return "\n".join(lines)
...
VAULT = a question answerable from the user's PERSONAL notes/vault ("the Brain") — their own history, family, pets, health, projects, past decisions ("when did Charlee start medication?", "what do my notes say about X", "pull up my note on Y", "read the Charlee health report", "search my vault for X", "find X in my brain"). Reading only, never saving — saving new content is NOTE."""
        ...
        try:
            if _is_status_cmd:
                ...
                raw_intent = await haiku(classify_prompt, response_schema=_INTENT_SCHEMA, label="chat_classify")
            intent = "CHAT"
            try:
