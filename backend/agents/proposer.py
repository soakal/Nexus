...
def _db_actionable_facts(limit: int = 12) -> list[dict]:
    ...
    try:
        ...
        for r in rows:
            ...
            if effective_confidence(r.confidence, age_days, source=r.source, pinned=r.pinned) < EFFECTIVE_FLOOR:
                continue
...
async def propose_goals_tick() -> dict:
    ...
    try:
        ...
        prompt = (
            ...
            "NOT destructive. Two kinds of goal are welcome:\n"
            "1) REACTIVE — something clearly warrants attention (storage near full, a device alert,\n"
            "   many stale PRs, a light left on, the garage door open, the back door unlocked,\n"
            "   a stopped VM or Docker container, an outage in UPTIME ANOMALIES).\n"
            "2) PROACTIVE MAINTENANCE — low-risk, reversible housekeeping worth doing even when\n"
            "   nothing is broken: investigate a disk/storage growth trend, verify a backup\n"
            "   actually completed, check a device that has not been reviewed recently, summarize\n"
            "   or organize knowledge from KNOWN FACTS, nudge stale GitHub PRs. Propose at most ONE\n"
            "   proactive-maintenance goal per response, and only when it is genuinely useful —\n"
            "   never invent busywork to fill the array.\n"
            "For anything MEDIUM/HIGH risk or irreversible, stay strictly reactive — only propose it\n"
            "when live state clearly shows the condition. Return an EMPTY array when nothing above\n"
            "applies. Never propose anything already open.\n"
