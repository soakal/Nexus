...
class Settings(BaseSettings):
    ...
    # quality set the planner/verifier back to "claude-opus-5" (or "claude-opus-4-8")
    # in .env. Any valid Anthropic model id works (billed to ANTHROPIC_API_KEY).
    # Migrated 2026-08-28: claude-sonnet-4-6 -> claude-sonnet-5, matching
    # backend/agents/router.py::SONNET_MODEL — see that file's _PRICE_PER_MTOK
    # comment for why (Anthropic made the $2/$10 intro rate permanent 2026-08-10).
    ...
    # Uses the Tailscale MagicDNS name (not a LAN IP) so taps from phone
    # notifications work from anywhere on the tailnet, not just at home.
    app_base_url: str = "https://nexus-lxc.tailfa52c.ts.net"
    ...
    # LXC path (docs/lxc-migration-spec.md:91) — distinct from Windows's frozen backup
    # share so backup_vault()'s rclone sync (destructive mirror) never wipes it.
    unraid_backup_path: str = r"\\192.168.1.50\Computer Backup\Nexus_backup-lxc"
