"""
Brain Organizer — TRIAL VARIANT for the 2026-08 OpenRouter model-swap trial.

This is a deliberate fork of brain_organizer.py, not an import of it — kept as
a plain copy so the diff against the real script stays reviewable in git, and
so nothing here can accidentally affect the production module. Only two real
changes from the production file, both inside _call_api:
  1. _call_openrouter() is the OpenRouter fallback block extracted verbatim
     into its own function (byte-identical behavior, just reusable).
  2. _call_api() now checks `"/" in model` FIRST and routes straight to
     _call_openrouter() for an OpenRouter-native model id (e.g.
     "google/gemini-2.5-pro") -- the production script only ever reaches
     OpenRouter as a same-model Anthropic-retry fallback, never as the
     primary target, so this branch cannot fire unless a trial config.json
     explicitly sets sonnet_model/haiku_model to an OpenRouter id.

MUST be run from an isolated working directory (never /opt/nexus/modules/
brain-organizer/ itself) -- _USAGE_LOG and the .organizer.lock path are both
__file__-relative, so running this file in place would silently share those
with the real, concurrently-running production script. See
/var/lib/nexus/brain-trial/run-trial.sh, which cp's this file into an
isolated directory before every run for exactly this reason.
...
def _call_api(
    ...
    *,
    ...
    # TRIAL: an OpenRouter-native model id (e.g. "google/gemini-2.5-pro") goes
    # straight to OpenRouter -- no Anthropic attempt, no retry loop. The
    # production script never reaches this: `model` there is always a bare
    # Anthropic model id (no "/"), so this branch is unreachable there.
    if "/" in model:
        or_key = os.environ.get("OPENROUTER_API_KEY", "")
        if not or_key:
            raise RuntimeError(f"OPENROUTER_API_KEY not set; cannot call OpenRouter model {model!r}")
        reasoning_max_tokens = config.get("reasoning_max_tokens")
        return _call_openrouter(model, messages, max_tokens, or_key, logger, reasoning_max_tokens=reasoning_max_tokens)
    ...
    try:
        return _call_openrouter(or_model, messages, max_tokens, or_key, logger)
    except _APIUsageCapped:
        raise
    except Exception as or_exc:
        if anthropic_usage_capped:
            raise _APIUsageCapped(f"Anthropic capped + OpenRouter also failed ({or_exc})") from or_exc
        raise RuntimeError(
...
def _call_openrouter(
    ...
    *,
    ...
    """The actual OpenRouter chat-completions call, extracted verbatim from the
    production script's fallback block so the TRIAL direct-routing path (see
    _call_api's `"/" in model` branch above) and the production fallback path
    share one implementation. Behavior is byte-identical to the pre-extraction
    inline version for a bare fallback call (reasoning_max_tokens=None) -- this
    is a pure refactor there, not a behavior change.

    `reasoning_max_tokens` (TRIAL-only, config-driven) caps a reasoning model's
    thinking budget -- Gemini 2.5 Pro counts thinking tokens against the SAME
    max_tokens budget as the visible answer, so an uncapped reasoning model can
    consume the whole max_tokens on thinking and return finish_reason="length"
    with zero visible text. Passed through as OpenRouter's "reasoning" param."""
    ...
    logger.info("OpenRouter call succeeded (finish_reason=%s)", finish_reason)
    ...
    # without this, a truncated OpenRouter response sailed past that guard
    # and got written as if it were complete.
