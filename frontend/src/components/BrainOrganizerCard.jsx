  ...
  return (
    <Card style={style}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: '12px', marginTop: '18px' }}>
        {/* Processed */}
        <div style={{ padding: '16px', borderRadius: '6px', background: 'rgba(52,211,153,0.07)', border: '1px solid rgba(52,211,153,0.18)' }}>
        ...
        {/* Pending */}
        <div style={{ padding: '16px', borderRadius: '6px', background: 'rgba(232,196,104,0.07)', border: '1px solid rgba(232,196,104,0.18)' }}>
          <div style={{ fontSize: '30px', fontWeight: 700, color: '#e8c468' }}>{data?.pending || 0}</div>
          <div style={{ fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#e8c468', fontWeight: 600, marginTop: '4px' }}>Pending</div>
        </div>
        {/* Failed */}
        <div style={{ padding: '16px', borderRadius: '6px', background: 'rgba(251,113,133,0.07)', border: '1px solid rgba(251,113,133,0.18)' }}>
      ...
      {/* Meta line */}
      <div style={{ fontSize: '12px', color: '#7a776d', marginTop: '14px' }}>
        Last run {lastRun} · Scheduled daily 2:00 AM
      </div>

      {/* Console log */}
      <div style={{ background: '#161615', border: '1px solid rgba(180,178,170,0.10)', borderRadius: '6px', padding: '14px 16px', maxHeight: '172px', overflow: 'auto', marginTop: '14px' }}>
        {(data?.log_tail || []).map((line, i) => {
          ...
          const tagColor = !tag ? 'var(--accent)'
            ...
            : 'var(--accent)'
          return (
            <div key={i} style={{ display: 'flex', gap: '10px', fontFamily: "'IBM Plex Mono',monospace", fontSize: '12px', lineHeight: 1.85 }}>
              {tag && (
                <span style={{ flex: 'none', fontWeight: 500, color: tagColor }}>[{tag}]</span>
              )}
              <span style={{ color: '#a6a399', wordBreak: 'break-word' }}>{msg}</span>
            </div>
          )
        })}
      </div>
