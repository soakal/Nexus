  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: 'linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0)),#1c1c1c',
        border: '1px solid rgba(255,138,61,0.22)',
        borderRadius: '6px',
        boxShadow: '0 24px 60px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,138,61,0.06)',
        overflow: 'hidden',
      }}>
        {/* Input */}
        <div style={{
          ...
          padding: '14px 16px',
          borderBottom: '1px solid rgba(180,178,170,0.10)',
        }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#98958c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <input
            ...
            style={{
              ...
              fontSize: '15px', color: '#f4f3f0',
              fontFamily: "'Archivo', system-ui, sans-serif",
            }}
          />
          <span style={{ fontSize: '11px', color: '#57554c', fontWeight: 600 }}>ESC</span>
        </div>

        {/* Results */}
        <div style={{ maxHeight: '340px', overflowY: 'auto' }}>
          {running && (
            <div style={{ padding: '16px', fontSize: '13px', color: '#98958c' }}>Running…</div>
          )}
          {result && !running && (
            <div style={{
              padding: '12px 16px', fontSize: '13px',
              color: result.ok ? '#34d399' : '#fb7185',
              borderBottom: '1px solid rgba(180,178,170,0.08)',
          ...
          {!running && items.length === 0 && (
            <div style={{ padding: '16px', fontSize: '13px', color: '#98958c' }}>No commands match.</div>
          )}
          {!running && items.map((item, i) => (
            <div
              ...
              style={{
                ...
                background: i === selected ? 'rgba(255,138,61,0.08)' : 'transparent',
                borderLeft: i === selected ? '2px solid var(--accent)' : '2px solid transparent',
              }}
            >
              <div>
                <div style={{ fontSize: '14px', fontWeight: 600, color: i === selected ? 'var(--accent)' : '#ece9e2' }}>
                  {item.label}
                </div>
                {item.hint && (
                  <div style={{ fontSize: '11px', color: '#7a776d', marginTop: '1px' }}>{item.hint}</div>
                )}
              </div>
              {item.to && (
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#57554c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Footer hint */}
        <div style={{
          padding: '8px 16px', borderTop: '1px solid rgba(180,178,170,0.08)',
          display: 'flex', gap: '16px',
        }}>
          {[['↑↓', 'navigate'], ['↵', 'run'], ['Esc', 'close']].map(([key, label]) => (
            <span key={key} style={{ fontSize: '11px', color: '#57554c' }}>
              <span style={{ color: '#98958c', fontWeight: 700 }}>{key}</span> {label}
            </span>
          ))}
        </div>
      </div>
