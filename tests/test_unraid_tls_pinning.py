...
def _run_on_selector_loop(coro):
    ...
    (reproduced independently while writing the sibling UniFi test suite).
    NEXUS itself never hits this: run.py pins SelectorEventLoop before
    uvicorn ever builds its loop, so production always runs this exact
    transport code under Selector, never Proactor.
    """
