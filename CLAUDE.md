...
- **Metering lives in `backend/agents/router.py`.** `_PRICE_PER_MTOK` holds `$/1M-token` placeholders per model — **`# VERIFY against current Anthropic pricing`** before trusting the dollar figures. `_compute_cost()` folds cache tokens into the input rate (approximation). `_record_spend()` is **best-effort**: wrapped in try/except, an absent/odd `usage` field or a DB failure NEVER crashes the LLM response. Usage whose token fields aren't a real numeric type (e.g. a `MagicMock` in tests) is treated as "no usage" and writes NO row. The spend write happens synchronously inside `_create_sync` — that runs in a `run_in_executor` worker thread (NOT the event loop), so a sync `Session` there is correct; do NOT "fix" it into `to_thread`.
...
- **Kill switch:** `SystemState.autonomy_enabled`. The broker (`execute_action`) checks it AFTER the idempotency replay but BEFORE classify/decide — when OFF, `agent`/`autonomous` actors get `FORBIDDEN` (logged with reason `autonomy_disabled`, no dispatch); `user` actions are unaffected. `worker_pool._worker_loop` has a NOTE to consult `autonomy_enabled` once AUTONOMOUS task generation lands (no behavior change today — current tasks are user-created).
...
- **New broker kind `hermes_action`** dispatches a *structured* verb: `classify` returns the verb's band; `_dispatch_hermes_action` calls `build_command` then `hermes.relay(command)` (a bad verb/args → ValueError → recorded FAILED, never re-raised). This is the ONLY way an `agent`/`autonomous` actor may command Hermes.
