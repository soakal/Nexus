...
- **Never block the asyncio event loop.** Windows ProactorEventLoop + a blocked loop = `WinError 64`, dropped connections, "everything offline". All sync DB work inside `async` funcs goes through `asyncio.to_thread`. The memo watcher starts on a daemon `threading.Thread`, not the loop. The SelectorEventLoop pin that used to work around this (and around ProactorEventLoop's separate `WinError 64` from CONCURRENT outbound httpx and the accept path) was Windows-only and has been removed — see lines 706-708/716: no event-loop pin is needed on this branch.
